        ...
        try:
            ...
            for event1, event2, time_diff in self.correlations:
                try:
                    ...
                    print(f"{event1.pcap_name} x={event1.x_pos}, {event2.pcap_name} x={event2.x_pos}")

            
                    
                    # Only draw if at least one point is visible
                    if -100 <= x1 <= self.width() + 100 or -100 <= x2 <= self.width() + 100:
...
class CrossPCAPTimelineWidget(QFrame):
    ...
    def _on_correlation_selected(self):
        ...
        # Handle correlation selection in the table
        try:
            ...
            if row < len(self.correlations):
                ...
                print(f"Selected correlation: {event1.value} between {event1.pcap_name} and {event2.pcap_name} (Δ{time_diff:.2f}s)")
