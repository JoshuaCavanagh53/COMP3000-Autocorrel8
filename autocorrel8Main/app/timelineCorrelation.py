...
class CrossPCAPTimelineWidget(QFrame):
    def __init__(self):
        ...
        self._sep_label            = None  # Divider between PCAP lanes and incognito lane
    ...
    def _init_ui(self):
        ...
        # Scroll area, vertical only — timeline is fixed to viewport width
        ...
        try:
            ...
            for pcap_name, events in sorted(timeline_data.items()):
                if events:
                    ...
                    self.timeline_layout.addWidget(tl)
    ...
    def _insert_incognito_lane(self):
        ...
        self._incognito_lane = IncognitoGapTimeline(
            self._incognito_gap_data, self.start_time, self.end_time,
        ...
        if idx >= 0:
            ...
            self.timeline_layout.insertWidget(idx + 1, self._incognito_lane)
        else:
    ...
    def _rebuild_timelines(self):
        ...
        for name, evs in sorted(data.items()):
            if evs:
                tl = PCAPTimeline(name, evs, self.start_time, self.end_time,
                                  pixels_per_second=self.pixels_per_second)
                self.pcap_timelines.append(tl)
                self.timeline_layout.addWidget(tl)
        self.timestamp_axis = TimestampAxis(self.start_time, self.end_time,
        ...
        self.timeline_layout.addWidget(self.timestamp_axis)
        self.timeline_layout.addStretch()
        # Re-insert the incognito lane if gap data was loaded before this rebuild
        self._insert_incognito_lane()

    def resizeEvent(self, event):
        # Re-fit the timeline whenever the widget is resized
