...
class RegistryWidget(QFrame):
    ...
    _COLS = ["Change", "Key Path", "Value Name", "Old Data", "New Data"]
    _COL_CHANGE = 0
    _COL_KEY = 1
    _COL_VALUE = 2
    _COL_OLD = 3
    ...
    def __init__(self):
        ...
        self._table.setColumnWidth(self._COL_VALUE, 130)
        self._table.setColumnWidth(self._COL_OLD, 150)
        self._table.setColumnWidth(self._COL_NEW, 150)
        self._table.horizontalHeader().setStretchLastSection(False)
 
        from PyQt5.QtWidgets import QHeaderView
        header = self._table.horizontalHeader()
        header.setSectionResizeMode(self._COL_KEY, QHeaderView.Stretch)
        for col in [self._COL_CHANGE, self._COL_VALUE, self._COL_OLD, self._COL_NEW]:
            header.setSectionResizeMode(col, QHeaderView.Stre)
    ...
    def _load_baseline(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Load Baseline Snapshot", "",
            "Snapshot Files (*.json);;All Files (*)"
        )
        if path:
